...
{
#define MAX_PROGRAM_HEADERS 5
    Elf32_Ehdr elf_header;
    Elf32_Phdr program_headers[MAX_PROGRAM_HEADERS];
    int read_size, ph_read_num, ph_counter;
    int file_section_offset, alignment_mask;
    void* physical_address;
    void* fd;

    fd = fl_fopen(image_path, "rb");
    if (0 == fd)
    {
        debug("Couldn't open kernel image. Aborting\n");
        return 0;
    }

    read_size = fl_fread(&elf_header, 1, sizeof(Elf32_Ehdr), fd);
    if (sizeof(Elf32_Ehdr) != read_size)
    {
        debug("Reading Elf header failed. Only read %d bytes. Aborting\n", read_size);
        return 0;
    }

    debug("Program header offset: %x\n", elf_header.e_phoff);
    debug("Number of program headers: %u\n", elf_header.e_phnum);

    read_size = fl_fseek(fd, elf_header.e_phoff, SEEK_SET);
    if (read_size!=0)
    {
        debug("Setting file position failed: %d\n", read_size);
        return 0;
    }

    ph_read_num = (MAX_PROGRAM_HEADERS<elf_header.e_phnum)?MAX_PROGRAM_HEADERS:elf_header.e_phnum;
    read_size = fl_fread(&program_headers, 1, ph_read_num * sizeof(Elf32_Phdr), fd);
    if (ph_read_num*sizeof(Elf32_Phdr) != read_size)
    {
        debug("Reading Program headers failed. Only read %d bytes. Aborting\n", read_size);
        return 0;
    }

    debug("Section: Type, Offset, Vaddr, Paddr, FSize, MSize, Flags, Align\n");
    for (ph_counter=0; ph_counter<ph_read_num; ph_counter++)
    {
        alignment_mask = program_headers[ph_counter].p_align;
        alignment_mask = ~(~alignment_mask & (alignment_mask-1));
        file_section_offset = program_headers[ph_counter].p_offset & alignment_mask;
        physical_address = program_headers[ph_counter].p_paddr & alignment_mask;
        fl_fseek(fd, file_section_offset, SEEK_SET);
        fl_fread(physical_address, 1, program_headers[ph_counter].p_filesz, fd);

        debug("Section: %x, %x, %x, %x, %x, %x, %x, %x\n",
              program_headers[ph_counter].p_type,
              program_headers[ph_counter].p_offset,
              program_headers[ph_counter].p_paddr,
              program_headers[ph_counter].p_vaddr,
              program_headers[ph_counter].p_filesz,
              program_headers[ph_counter].p_memsz,
              program_headers[ph_counter].p_flags,
              program_headers[ph_counter].p_align);
    }

    typedef void (*kernelstart)(void);
    kernelstart kernel;
    kernel = (kernelstart)(elf_header.e_entry);
    kernel();
...
struct module * load_module(void *mod_image, unsigned long len)
{
	...
	for (i = 0; i < ehdr->e_shnum; i++) {
		...
		//debug("%d addr: 0x%x size: 0x%x ofs: 0x%x\n",
